...
import { updateContractsIVField, addContractsIVFiles, removeContractsIVFile, ExistingContractsIVState } from '../../../../../store/slices/financialInfoSlice';

const SubStep2: React.FC = () => {
  const dispatch = useAppDispatch();
  const existingContractsIVState = useAppSelector((state) => state.financialInfo.existingContractsIV);
  const { hasSteamContract, rate, escalator, offtakeRequirement, condensateReturn, condensatePercentage, fileMetadata } = existingContractsIVState;
  
  const handleUpdateField = (field: keyof Omit<ExistingContractsIVState, 'files' | 'fileMetadata'>, value: string | boolean) => {
      dispatch(updateContractsIVField({ field, value }));
  };
  ...
  const handleRateChange = (value: string) => {
    ...
    handleUpdateField('rate', cleaned);
  };
  ...
  const handleRateBlur = () => {
    ...
    if (!isNaN(parsed)) {
      ...
      handleUpdateField('rate', formatted);
    } else {
      handleUpdateField('rate', '');
    }
  };
  ...
  const handleEscalatorChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^([1-9](\.\d{0,2})?)?$/.test(value) || value === '') {
      handleUpdateField('escalator', value);
    }
  };

  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => { 
      if (event.target.files) {
        dispatch(addContractsIVFiles(Array.from(event.target.files)));
      } 
  };
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => { 
      ...
      if (event.dataTransfer.files) {
        dispatch(addContractsIVFiles(Array.from(event.dataTransfer.files)));
      }
  };
  ...
  const handleRemoveFile = (fileName: string) => {
      dispatch(removeContractsIVFile(fileName));
  };
        ...
        <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 2, pt: '10px', pb: '10px', pl: '160px', pr: '160px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Tooltip title="Steam contracts are a third-party agreement to sell therms (in MLBs of steam)..." placement="right-start" arrow>
              <FormControlLabel control={<Switch checked={hasSteamContract} onChange={(e) => handleUpdateField('hasSteamContract', e.target.checked)} size="small" />} label="Do you have any existing Steam Contracts?" sx={{ '& .MuiFormControlLabel-label': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.9rem' } }} />
            </Tooltip>
          </Box>
          {hasSteamContract && (
            <Box sx={{ mb: 0, pl: 0 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 0 }}>
                <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 2, pt: '10px', pb: '10px' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <TextField variant="outlined" size="small" type="number" placeholder="Input" value={offtakeRequirement} onChange={(e) => handleUpdateField('offtakeRequirement', e.target.value)} sx={{ flex: 0.5, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { height: '40px', padding: '0 6px' }, '& input': { padding: 0, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem' }, '& .MuiInputBase-input::placeholder': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem' } }} />
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Select size="small" id="condensate-select" value={condensateReturn} onChange={(e: SelectChangeEvent) => handleUpdateField('condensateReturn', e.target.value)} sx={{ flex: 0.5, marginLeft: 'auto', fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', height: '40px', '& .MuiInputBase-root': { padding: '0 6px' }, '& .MuiSelect-select': { padding: '4px 6px', fontSize: '0.7rem' } }}>
                  ...
                  {condensateReturn === 'yes' && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <TextField variant="outlined" size="small" type="number" placeholder="Enter percentage" value={condensatePercentage} onChange={(e) => handleUpdateField('condensatePercentage', e.target.value)} InputProps={{ endAdornment: <InputAdornment position="end">%</InputAdornment> }} sx={{ flex: 0.5, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { height: '40px', padding: '0 6px' }, '& input': { padding: 0, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem' }, '& .MuiInputBase-input::placeholder': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem' } }} />
                    </Box>
                  )}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Typography sx={{ mt: 1, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.75rem', minWidth: '200px', flex: 1 }}>
              <i><b>* </b>If <b>Yes</b>, please provide details in the fields or upload the agreement and EmissionCheckIQ+ will autopopulate the fields for you.</i>
            </Typography>
          </Box>
        </Box>
      </Box>
