...
const SubStep2: React.FC = () => {
  ...
  const handleChange = (value: string, index: number) => {
    ...
    updateYearBudget(index, updatedValue);
  };

  const handleBlur = (index: number) => {
    const num = parseFloat(yearBudget[index]);
    if (!isNaN(num)) {
      updateYearBudget(index, currencyFormatter.format(num));
    } else {
      updateYearBudget(index, '');
    }
  };
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, opacity: availableFunds !== 'yes' ? 0.5 : 1 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 0.5, maxWidth: 'calc(100% - 200px)' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, ml: 1 }}>
              <IconButton onClick={addYearBudget} color="primary" disabled={availableFunds !== 'yes'} sx={{ p: 0, '&:focus': { outline: 'none' } }}><AddCircleOutlineIcon fontSize="small" /></IconButton>
              <IconButton onClick={() => removeYearBudget(yearBudget.length - 1)} sx={{ p: 0, '&:focus': { outline: 'none' } }} disabled={availableFunds !== 'yes' || yearBudget.length === 1}><DeleteIcon fontSize="small" /></IconButton>
            </Box>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Select size="small" value={simplePayback} onChange={(e: SelectChangeEvent) => updateField('simplePayback', e.target.value)} sx={{ flex: 0.5, fontFamily: 'Nunito Sans, sans-serif', marginLeft: 'auto', fontSize: '0.7rem', height: '40px', '& .MuiInputBase-root': { padding: '0 6px' }, '& .MuiSelect-select': { padding: '4px 6px', fontSize: '0.7rem' } }}>
