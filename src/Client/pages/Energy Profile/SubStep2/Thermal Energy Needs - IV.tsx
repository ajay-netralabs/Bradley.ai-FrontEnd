        ...
        <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 2, pt: '10px', pb: '10px', pl: '160px', pr: '160px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Tooltip title="Click to expand or move on to the next step." placement='right' arrow>
              <FormControlLabel
                control={<Switch checked={showWasteHeat} onChange={(e) => updateField('showWasteHeat', e.target.checked)} size="small" />}
            ...
            <Box>
              {wasteHeatSources.map((source, index) => (
                <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  <TextField size="small" label="*Type" select value={source.type} onChange={(e) => updateWasteHeatSourceField(index, 'type', e.target.value)} sx={{ flex: 1, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem', height: '40px' } }} InputLabelProps={{ style: { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem' } }}>
                  ...
                  <TextField size="small" label="*Temp. (°F)" type="number" value={source.temperature} onChange={(e) => updateWasteHeatSourceField(index, 'temperature', e.target.value)} sx={{ flex: 1, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', height: '40px' } }} InputLabelProps={{ style: { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem' } }} />
                  <TextField size="small" label="Flow Rate" placeholder="BTU/hr, lbs/hr" value={source.flowRate} onChange={(e) => updateWasteHeatSourceField(index, 'flowRate', e.target.value)} sx={{ flex: 1, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', height: '40px' } }} InputLabelProps={{ style: { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem' } }} />
                  <TextField size="small" label="Utilization" select value={source.utilization} onChange={(e) => updateWasteHeatSourceField(index, 'utilization', e.target.value)} sx={{ flex: 1, fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', '& .MuiInputBase-root': { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.7rem', height: '40px' } }} InputLabelProps={{ style: { fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem' } }}>
                  ...
                  <IconButton onClick={() => removeWasteHeatSource(index)} size="small" disabled={wasteHeatSources.length === 1} sx={{ '&:focus': { outline: 'none' } }}>
                    <DeleteIcon fontSize="small" />
                  </IconButton>
                </Box>
              ))}

              <Button startIcon={<AddCircleIcon />} onClick={addWasteHeatSource} size="small" sx={{ textTransform: 'none', fontFamily: 'Nunito Sans, sans-serif', fontSize: '0.8rem', '&:focus': { outline: 'none' } }}>
                Add Another Entry
              </Button>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 4, mt: 2 }}>
                ...
                <Select
                  ...
                  onChange={(e: SelectChangeEvent) => updateField('benefitFromUtilization', e.target.value)}
